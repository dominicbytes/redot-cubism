...
public partial class demo_effect_custom_03 : Node2D
{
    ...
    private Godot.Collections.Array<GDCubismParameterCS> ary_parameters = new();
    private Godot.Collections.Array<float> ary_volume_history = new();
    ...
    private void _on_cubism_prologue(Node2D _model, double v) {}
    private void _on_cubism_epilogue(Node2D _model, double v) {}
    private void free_parameters()
    {
        foreach (GDCubismParameterCS param in this.ary_parameters)
        {
            param.Free();
        }

        this.ary_parameters.Clear();
        this.param_mouth = null;
    }

    private void _on_cubism_init(Node2D _model)
    {
        this.free_parameters();
        this.ary_volume_history.Clear();
        this.history_position = 0;

        this.ary_parameters = this.cubism_model.GetParameters();
        foreach (GDCubismParameterCS param in this.ary_parameters)
        {
        ...
        int bus_index = AudioServer.GetBusIndex(this.audio_bus_name);
        this.spectrum = AudioServer.GetBusEffectInstance(bus_index, this.audio_efx_index) as AudioEffectSpectrumAnalyzerInstance;
        this.lipsync_ready = this.spectrum != null;
    }
    ...
    public override void _Notification(int what)
    {
        ...
        if (this.cubism_efx != null)
        {
            ...
            this.cubism_efx.CubismTerm -= this._on_cubism_term;
            this.cubism_efx.CubismProcess -= this._on_cubism_process;
            this.cubism_efx.Free();
            this.cubism_efx = null;
        }

        this.free_parameters();

        if (this.cubism_model != null)
        {
            this.cubism_model.Free();
            this.cubism_model = null;
        }
    }
