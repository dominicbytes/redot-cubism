...
func _run() -> void:
	...
	assert(save_dialog.get_option_count() == 5)
	assert(save_dialog.get_option_name(3) == "Mask quality" and save_dialog.get_option_default(3) == 1)
	assert(save_dialog.get_option_name(4) == "Premultiplied alpha" and save_dialog.get_option_default(4) == 0)
	...
	save_dialog.set_option_default(4, 1)
	source_dialog.file_selected.emit(source)
	...
	assert(menu_model.get_premultiplied_alpha())
