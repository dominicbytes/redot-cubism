...
func _run() -> void:
	...
	if "--overlay-checks" in OS.get_cmdline_user_args():
		...
		get_tree().quit(0 if passed else 1)
		return
	if "--transform-checks" in OS.get_cmdline_user_args():
		var passed: bool = await preload("res://renderer_transform_checks.gd").run(self, fixture)
		get_tree().quit(0 if passed else 1)
		return
	if "--offscreen-checks" in OS.get_cmdline_user_args():
		var passed: bool = await preload("res://renderer_offscreen_checks.gd").run(self, fixture)
		get_tree().quit(0 if passed else 1)
		return
	if "--overlap-checks" in OS.get_cmdline_user_args():
		var passed: bool = await preload("res://renderer_overlap_checks.gd").run(self, fixture)
		get_tree().quit(0 if passed else 1)
		return
